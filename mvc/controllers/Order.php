...
class order extends ControllerBase
{
    ...
    public function message($message)
    {
        if ($message == "success") {
            $this->view("client/message", [
                ...
                "thanks" => "Cuộc sống có nhiều lựa chọn, cảm ơn vì bạn đã chọn và tin tưởng SYDAPHAM STORE!"
            ]);
        } else {
            $this->view("client/message", [
                "headTitle" => "Thông báo",
    ...
    public function payment($orderId = "", $amount = "", $paymentMethod = "")
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($paymentMethod == "vnpay") {
                ...
                $this->handlePaymentWithVNPAY($vnp_Amount, $vnp_TxnRef, $vnp_Returnurl);
    ...
    private function handlePaymentWithVNPAY($vnp_Amount, $vnp_TxnRef, $vnp_Returnurl)
    {
        $startTime = date("YmdHis");
        $expire = date('YmdHis',strtotime('+15 minutes',strtotime($startTime)));
        $vnp_TmnCode = 'USF2T93R';
        $vnp_HashSecret = 'AISSGFLZMPJSYSXHRDJVGPXKFMEZAKRJ';
