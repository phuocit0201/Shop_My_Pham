...
class userModel
{
    ...
    public function checkLogin($email, $password)
    {
        ...
        $sql = "SELECT u.id, u.fullName, r.name AS RoleName FROM users u JOIN role r ON u.roleId = r.id WHERE email='$email' AND password='$md5Password' AND isConfirmed=1 AND status = 1 AND delete_flg = 0";
    ...
    public function insert($fullName, $email, $dob, $address, $password, $provinceId, $districtId, $wardId)
    {
        ...
        if ($result) {
            ...
            $mail->SetFrom("pn.flatshop@gmail.com", "SYDAPHAM STORE");
            $mail->Subject = "Xác nhận email tài khoản - SYDAPHAM STORE";
            $mail->Body = "<h3>Cảm ơn bạn đã đăng ký tài khoản tại website SYDAPHAM STORE</h3></br>Đây là mã xác minh tài khoản của bạn: " . $captcha . "";
