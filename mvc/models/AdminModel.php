...
class adminModel
{
    ...
    public function getAllAdmin($page = 1, $total = 8)
    {
        ...
        $tmp = ($page - 1) * $total;
        $db = DB::getInstance();
        $sql = "SELECT * FROM users where roleID = 1 and delete_flg = 0 LIMIT $tmp,$total";
    ...
    {
        $db = DB::getInstance();
        $sql = "SELECT * FROM users where roleID = 1 and delete_flg = 0 and id='$Id'";
        $result = mysqli_query($db->con, $sql);
        return $result;
    }
    ...
    {
        $db = DB::getInstance();
        $sql = "SELECT COUNT(*) FROM users where roleID = 1 and delete_flg = 0";
        $result = mysqli_query($db->con, $sql);
        if ($result) {
